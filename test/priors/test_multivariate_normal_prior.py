import torch
from gpytorch_mini.priors.torch_priors import MultivariateNormalPrior
from gpytorch_mini.utils.test.base_test_case import BaseTestCase
from torch.distributions import MultivariateNormal


class TestMultivariateNormalPrior(BaseTestCase):
    ...
    def test_multivariate_normal_prior_log_prob_log_transform(self, cuda=False):
        ...
        cov = torch.eye(2, device=device)
        prior = MultivariateNormalPrior(
            mean, covariance_matrix=cov, transform=torch.exp
        )
        dist = MultivariateNormal(mean, covariance_matrix=cov)
