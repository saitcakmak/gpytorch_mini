...
from gpytorch_mini.mlls.marginal_log_likelihood import MarginalLogLikelihood
...
class SumMarginalLogLikelihood(MarginalLogLikelihood):
    ...
    def forward(self, outputs, targets, *params):
        ...
        if len(params) == 0:
            sum_mll = sum(
                mll(output, target)
                for mll, output, target in zip(self.mlls, outputs, targets, strict=True)
            )
        else:
            sum_mll = sum(
                mll(output, target, *iparams)
                for mll, output, target, iparams in zip(
                    self.mlls, outputs, targets, params, strict=True
                )
            )
