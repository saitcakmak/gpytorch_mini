...
def sq_dist(x1, x2, x1_eq_x2=False):
    ...
    # NOTE: the linked PR landed but it does not implement squared dist.
...
def dist(x1, x2, x1_eq_x2=False):
    """
    # TODO: this statement is questionable. Why do we need a separate implementation if they're equivalent?
...
class Kernel(Module):
    r"""
    Kernels in GPyTorch are implemented as a :class:`gpytorch_mini.Module` that, when called on two :class:`torch.Tensor`
    ...
    :py:meth:`~gpytorch_mini.kernels.Kernel.forward` method.

    .. note::
        The :py:meth:`~gpytorch_mini.kernels.Kernel.__call__` method does some additional internal work. In particular,
    ...
    * ARD: Each input dimension gets its own separate lengthscale
    ...
    :param lengthscale_constraint: Set this if you want to apply a constraint
        to the lengthscale parameter. (Default: :class:`~gpytorch_mini.constraints.Positive`.)
    ...
    Example:
        >>> covar_module = gpytorch_mini.kernels.LinearKernel()
        >>> x1 = torch.randn(50, 3)
    ...
    def forward(
        ...
        x1: Tensor,
        x2: Tensor,
        diag: bool = False,  # TODO: Do we care for this?
        last_dim_is_batch: bool = False,  # TODO: same here
        **params,  # TODO:
    ) -> Tensor:
        r"""
    ...
    def local_load_samples(  # TODO: what is this?
    ...
    def expand_batch(
        self, *sizes: Union[torch.Size, Tuple[int, ...]]
    ) -> Kernel:  # TODO: is this necessary? Doesn't seem so.
        r"""
    ...
    def named_sub_kernels(self) -> Iterable[Tuple[str, Kernel]]:
        """
        For compositional Kernel classes (e.g. :class:`~gpytorch_mini.kernels.AdditiveKernel`
        or :class:`~gpytorch_mini.kernels.ProductKernel`).
    ...
    def __call__(
        ...
        **params,
        ...
        .. note::
            Following PyTorch convention, all :class:`~gpytorch_mini.models.GP` objects should use `__call__`
            rather than :py:meth:`~gpytorch_mini.kernels.Kernel.forward`.
        ...
        # TODO: This is a useful check but do we want it on the primary evaluation path?
        ...
        return super(Kernel, self).__call__(
            x1_, x2_, diag=diag, last_dim_is_batch=last_dim_is_batch, **params
        )
