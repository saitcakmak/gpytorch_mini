...
class LKJCholeskyFactorPrior(Prior, LKJCholesky):
    ...
    def __init__(self, n, eta, validate_args=False, transform=None):
        ...
        LKJCholesky.__init__(
            self, dim=n, concentration=eta, validate_args=validate_args
        )
        self.n = self.dim
...
class LKJPrior(LKJCholeskyFactorPrior):
    ...
    def log_prob(self, X):
        ...
        X_cholesky = psd_safe_cholesky(X)
        return super().log_prob(X_cholesky)

    def sample(self, sample_shape=EMPTY_SIZE):
...
class LKJCovariancePrior(LKJPrior):
    ...
    def __init__(self, n, eta, sd_prior, validate_args=False):
        ...
        if sd_prior.event_shape not in {
            torch.Size([1]),
            torch.Size([n]),
            torch.Size([]),
        }:
    ...
    def sample(self, sample_shape=EMPTY_SIZE):
        ...
        marginal_sds = marginal_sds.repeat(
            *[1] * len(sample_shape), self.correlation_prior.n
        )
...
def _is_valid_correlation_matrix(Sigma, tol=1e-6):
    ...
    if not torch.all(evals >= -tol):
        return False
    return all(
        torch.all(torch.abs(S.diagonal(dim1=-1, dim2=-2) - 1) < tol)
