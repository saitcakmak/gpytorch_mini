...
class BaseTestCase(TestCase, ABC):
    ...
    def assertAllClose(self, tensor1, tensor2, rtol=1e-4, atol=1e-5, equal_nan=False):
        if not tensor1.shape == tensor2.shape:
            raise ValueError(
                f"tensor1 ({tensor1.shape}) and tensor2 ({tensor2.shape}) do not have the same shape."
            )
        ...
        atol_diff = (torch.abs(tensor1 - tensor2) - torch.abs(tensor2).mul(rtol)).view(
            -1
        )
    ...
    def assertEqual(self, item1, item2):
        if torch.is_tensor(item1) and torch.is_tensor(item2):
            ...
            raise AssertionError(
                f"item1 ({type(item1)}) and item2 ({type(item2)}) are not the same type."
            )
        elif item1 == item2:
            return True
