...
class BaseKernelTestCase(ABC):
    ...
    def test_active_dims_list(self):
        ...
        covar_mat_actual = kernel_basic(x[:, [0, 2, 4, 6]])
    ...
    def test_active_dims_range(self):
        ...
        covar_mat = kernel(x)
        kernel_basic = self.create_kernel_no_ard()
        covar_mat_actual = kernel_basic(x[:, active_dims])
        self.assertAllClose(covar_mat, covar_mat_actual, rtol=1e-3, atol=1e-5)

    def test_no_batch_kernel_single_batch_x_no_ard(self):
        kernel = self.create_kernel_no_ard()
        x = self.create_data_single_batch()
        batch_covar_mat = kernel(x)

        actual_mat_1 = kernel(x[0])
        actual_mat_2 = kernel(x[1])
        actual_covar_mat = torch.cat(
            [actual_mat_1.unsqueeze(0), actual_mat_2.unsqueeze(0)]
        )
    ...
    def test_no_batch_kernel_double_batch_x_no_ard(self):
        ...
        for i in range(x.size(0)):
            ...
            for j in range(x.size(1)):
                i_actual_covars.append(kernel(x[i, j]))
            ij_actual_covars.append(
                torch.cat([ac.unsqueeze(0) for ac in i_actual_covars])
            )
    ...
    def test_kernel_getitem_single_batch(self) -> None:
        kernel = self.create_kernel_no_ard(batch_shape=torch.Size([2]))
        x = self.create_data_single_batch()

        res1 = kernel(x)[0]  # Result of first kernel on first batch of data

        new_kernel = kernel[0]
        res2 = new_kernel(
            x[0]
        )  # Should also be result of first kernel on first batch of data.

        self.assertAllClose(res1, res2, rtol=1e-3, atol=1e-5)

    def test_kernel_getitem_double_batch(self) -> None:
        kernel = self.create_kernel_no_ard(batch_shape=torch.Size([3, 2]))
        x = self.create_data_double_batch()

        res1 = kernel(x)[0, 1]  # Result of first kernel on first batch of data
    ...
    def test_kernel_getitem_broadcast(self) -> None:
        kernel = self.create_kernel_no_ard(batch_shape=torch.Size([2]))
        x = self.create_data_double_batch()
        kernel = kernel.expand_batch(
            torch.broadcast_shapes(kernel.batch_shape, x.shape[:-2])
        )

        idx1 = torch.LongTensor([0, 2, 1]).unsqueeze(-1)
        idx2 = torch.LongTensor([1, 0, 0]).unsqueeze(-2)
        res1 = kernel(x)[idx1, idx2]  # Result of first kernel on first batch of data
    ...
    def test_kernel_pickle_unpickle(self) -> None:
        kernel = self.create_kernel_no_ard(batch_shape=torch.Size([]))
        pickle.loads(
            pickle.dumps(kernel)
        )  # Should be able to pickle and unpickle a kernel

    def test_kernel_dtype_device(self) -> None:
        kernel = self.create_kernel_no_ard(batch_shape=torch.Size([]))
        ...
        if not kernel.has_lengthscale:
            kernel.register_parameter(
                "foo", torch.nn.Parameter(torch.zeros(1, dtype=orig_type))
            )
            with self.assertRaisesRegex(
                RuntimeError, "parameters have multiple dtypes"
            ):
                kernel.dtype
