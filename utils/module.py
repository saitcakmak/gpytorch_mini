...
class Module(nn.Module):
        ...
        for name, val in kwargs.items():
            ...
            if "." in name:
                ...
                if (
                    ...
                    and not constraint.check_raw(val)
                ):
                    raise RuntimeError(
                        ...
                        "(noise_constraint=gpytorch_mini.constraints.GreaterThan(better_lower_bound))"
                    )
                try:
                    self.__getattr__(name).data.copy_(
                        val.expand_as(self.__getattr__(name))
                    )
                except RuntimeError:
                    if not self._strict_init:
                        ...
                        self.__getattr__(name).data.copy_(
                            val.view_as(self.__getattr__(name))
                        )
    ...
    def register_prior(self, name, prior, param_or_closure, setting_closure=None):
        ...
        if isinstance(param_or_closure, str):
            if param_or_closure not in self._parameters and not hasattr(
                self, param_or_closure
            ):
                raise AttributeError(
            ...
            if len(inspect.signature(param_or_closure).parameters) == 0:
                raise ValueError(
                    ...
                    likelihood.noise_covar.register_prior(
                        "noise_std_prior",
                        gpytorch_mini.priors.NormalPrior(0, 1),
                        lambda module: module.noise.sqrt()
                    )
                    """
                )
            if (
                inspect.isfunction(setting_closure)
                and len(inspect.signature(setting_closure).parameters) < 2
            ):
                raise ValueError(
                    ...
                    kernel.register_prior(
                        "radius_prior",
                        gpytorch_mini.priors.LogNormalPrior(0, 1),
    ...
    def constraint_for_parameter_name(self, param_name):
        ...
        except AttributeError:  # submodule may not always be a gpytorch_mini module
            return None
    ...
    def to_random_module(self):
        random_module_cls = type(
            "_Random" + self.__class__.__name__, (RandomModuleMixin, self.__class__), {}
        )
    ...
    def local_load_samples(self, samples_dict, memo, prefix):
        ...
        """
        self._strict(False)
        for name, (prior, _closure, setting_closure) in self._priors.items():
            if prior is not None and prior not in memo:
                memo.add(prior)
                setting_closure(
                    self, samples_dict[prefix + ("." if prefix else "") + name]
                )
        self._strict(True)
    ...
    def pyro_load_from_samples(self, samples_dict):
        ...
        """
        return _pyro_load_from_samples(
            module=self, samples_dict=samples_dict, memo=None, prefix=""
        )
...
def _validate_module_outputs(outputs):
    if isinstance(outputs, tuple):
        if not all(
            torch.is_tensor(output) or isinstance(output, Distribution)
            for output in outputs
        ):
            raise RuntimeError(
                "All outputs must be a Distribution or torch.Tensor. "
        ...
        raise RuntimeError(
            "Output must be a Distribution or torch.Tensor. Got {}".format(
...
def _set_strict(module, value, memo=None):
    ...
    for _, module_ in module.named_children():
    ...
    if hasattr(module, "_priors"):
        for prior_name, (prior, closure, setting_closure) in module._priors.items():
            if prior is not None and prior not in memo:
                ...
                value = pyro.sample(
                    prefix + ("." if prefix else "") + prior_name, prior
                )
...
def _pyro_load_from_samples(module, samples_dict, memo=None, prefix=""):
    ...
    for mname, module_ in module.named_children():
        ...
        _pyro_load_from_samples(
            module_, samples_dict, memo=memo, prefix=submodule_prefix
        )
...
def _extract_named_added_loss_terms(module, memo=None, prefix=""):
    ...
    for mname, module_ in module.named_children():
        submodule_prefix = prefix + ("." if prefix else "") + mname
        for name, strategy in _extract_named_added_loss_terms(
...
def _extract_named_priors(module, prefix=""):
    ...
    for mname, module_ in module.named_children():
        ...
        for name, parent_module, prior, closure, inv_closure in _extract_named_priors(
            module_, prefix=submodule_prefix
        ):
...
def _extract_named_constraints(module, memo=None, prefix=""):
    ...
    for mname, module_ in module.named_children():
        submodule_prefix = prefix + ("." if prefix else "") + mname
        for name, constraint in _extract_named_constraints(
    ...
    def initialize(self, **kwargs):
        ...
        for name, value in kwargs.items():
            if not torch.is_tensor(value):
                raise RuntimeError(
                    "Initialize in RandomModules can only be done with tensor values."
                )
            ...
            is_property = hasattr(type(self), name) and isinstance(
