...
import torch
from gpytorch_mini.constraints.transform_utils import (
...
class Interval(Module):
    def __init__(
        ...
        lower_bound = torch.as_tensor(lower_bound, dtype=dtype)
        upper_bound = torch.as_tensor(upper_bound, dtype=dtype)
    ...
    def enforced(self) -> bool:
        # TODO: seems like this determines whether to transform. It's a weird name.
        return self._transform is not None

    def check(self, tensor) -> bool:
        return bool(
            torch.all(tensor <= self.upper_bound)
            and torch.all(tensor >= self.lower_bound)
        )
    ...
    def transform(self, tensor: Tensor) -> Tensor:
        ...
        transformed_tensor = (
            self._transform(tensor) * (self.upper_bound - self.lower_bound)
        ) + self.lower_bound
    ...
    def inverse_transform(self, transformed_tensor: Tensor) -> Tensor:
        ...
        tensor = self._inv_transform(
            (transformed_tensor - self.lower_bound)
    ...
    def __repr__(self) -> str:
        if self.lower_bound.numel() == 1 and self.upper_bound.numel() == 1:
            return (
                self._get_name() + f"({self.lower_bound:.3E}, {self.upper_bound:.3E})"
            )
        else:
...
class GreaterThan(Interval):
    ...
    def transform(self, tensor: Tensor) -> Tensor:
        transformed_tensor = (
            self._transform(tensor) + self.lower_bound if self.enforced else tensor
        )
        return transformed_tensor

    def inverse_transform(self, transformed_tensor: Tensor) -> Tensor:
        tensor = (
            self._inv_transform(transformed_tensor - self.lower_bound)
            if self.enforced
            else transformed_tensor
        )
        return tensor
...
class Positive(GreaterThan):
    def __init__(
        self, transform=softplus, inv_transform=inv_softplus, initial_value=None
    ):
        super().__init__(
            lower_bound=0.0,
            transform=transform,
    ...
    def inverse_transform(self, transformed_tensor: Tensor) -> Tensor:
        tensor = (
            self._inv_transform(transformed_tensor)
            if self.enforced
            else transformed_tensor
        )
        return tensor
...
class LessThan(Interval):
    ...
    def transform(self, tensor: Tensor) -> Tensor:
        transformed_tensor = (
            -self._transform(-tensor) + self.upper_bound if self.enforced else tensor
        )
        return transformed_tensor

    def inverse_transform(self, transformed_tensor: Tensor) -> Tensor:
        tensor = (
            -self._inv_transform(-(transformed_tensor - self.upper_bound))
            if self.enforced
            else transformed_tensor
        )
        return tensor
